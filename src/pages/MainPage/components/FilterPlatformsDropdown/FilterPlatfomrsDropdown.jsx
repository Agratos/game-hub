...
const FilterPlatfomrsDropdown = ({ORDER_ARR}) => {
  ...
  const findName = () => {
    let result = PLATFORMS.filter((el) => el.id === Number(query.get("parent_platforms")));
    console.log(result)
  ...
  return (
    <Dropdown>
      <Dropdown.Toggle variant="dark" id="dropdown-basic" className='p-1'>
        <span style={{fontSize: "0.8rem"}}>{query.get("parent_platforms") ? findName() : "Platforms"}</span>
      </Dropdown.Toggle>

      <Dropdown.Menu>
        {PLATFORMS.map((item, index) => 
          <Dropdown.Item onClick={() => goOrdering(item.id)} key={index}>{item.name}</Dropdown.Item>
        )}
    </Dropdown.Menu>
    </Dropdown>
  )
