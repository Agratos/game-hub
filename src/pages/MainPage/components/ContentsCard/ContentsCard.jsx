...
const ContentsCard = ({item}) => {
  ...
  return (
      <figure className='mainpage-card-area'style={{ width: '18rem'}} onMouseEnter={handleThumbnail} onMouseLeave={() => setIsImgList(false)}>
          {item?.short_screenshots?.length > 0 && isImgList ?
            <CardImgSlide cardList={item.short_screenshots}/>
          : <img className='mainpage-card-slide-img' style={{height:'10rem'}} src={Array.isArray(item?.tags) && handleImage() ?  item?.background_image : "/images/emptyImage.webp"} alt="card img"/>
          }
